...
// ═══════════════════════════════════════════════════════════════════════════════
//  Klaus Fish Bowl Chat — v7.0.04
//  GitHub : https://github.com/backwater-battery/Dadders-cybertube
//  jsDelivr: https://cdn.jsdelivr.net/gh/backwater-battery/Dadders-cybertube@main/Klausfishbowlchat-7.0.04.js
//
//  CHANGES v7.0.04:
//    • Userlist polling no longer logs every 2s — only logs when count changes
//    • MutationObserver on userlist no longer double-fires with the interval
//    • Log noise reduced — routine silent polls are silent
//    • All core features confirmed working:
//        ✓ Bubbles spawn with correct username + message
//        ✓ Emoji in messages
//        ✓ $server$ / nick-highlight system messages filtered
//        ✓ Userlist pills render with CyTube colours
//        ✓ Send via window.socket.emit confirmed working
// ═══════════════════════════════════════════════════════════════════════════════
...
(function () {
  ...
  function init() {
    ...
    statusMsg = "running";
    dbg("v7.0.04 init OK — socket ready");
    setup();
  }
  ...
  function setup() {
    ...
    // ── Userlist sync ─────────────────────────────────────────────────────
    // Confirmed structure (util.js addUserToList):
    //   #userlist > div.userlist_item
    //     children[0] = rank icon span
    //     children[1] = name span (plain, no class)
    //
    // Only logs when user count changes — no more 2s flood in the log.
    let lastUserCount = -1;
    ...
    function refreshUserList(forceLog) {
      ...
      // Only log on change (or forced on first run / join / leave)
    ...
    // Interval as safety net for edge cases — silent unless count changes
    setInterval(() => refreshUserList(false), 3000);

    // MutationObserver for instant join/leave response
    const ulEl = document.getElementById("userlist");
    if (ulEl) {
      new MutationObserver(() => refreshUserList(true))
    ...
    //
    //  Skip classes (plain string includes — avoids $ regex escaping issues):
    //    "nick-highlight"   — mention highlights
    //    "server-msg"       — server status messages
    //    "$server$"         — CyTube system user (/clear, voteskip, etc)
    //
    //  Skip usernames: $server$, $voteskip$, $poll$
    //
    ...
    function processNode(node) {
      ...
      // Skip system/server messages
      if (SKIP_CLASS_FRAGMENTS.some(f => cls.includes(f))) {
        dbg("skip: " + cls.substring(0,50));
        return;
      }
      ...
      if (!name) {
        ...
        if (idx !== -1) {
          name = cls.slice(idx + prefix.length).split(" ")[0];
      ...
      if (SKIP_NAMES.includes(name)) { dbg("  skip name: " + name); return; }

      // Extract message — last span that isn't timestamp or name wrapper
