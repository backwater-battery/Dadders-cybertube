...
// ─── DEBUG MODE — set false to hide the Klaus's Logs button entirely ─────────
...
(function () {
  ...
  // ─── Klaus's Logs ─────────────────────────────────────────────────────────
  // A collapsible log panel triggered by a floating button.
  // Buttons: Copy | Status | Clear | Close
  let logPanel  = null;
  let logBody   = null;
  let logToggle = null;
  let logLines  = [];   // raw text lines for copy
  let statusMsg = "idle";
  ...
  function buildLogUI() {
    ...
    // Status button — shows current statusMsg in an alert-style overlay
    const statusBtn = makeBtn("Status", "#7dd8ff", () => {
      showStatusOverlay();
    });
  ...
  // Status overlay — small card over the panel showing current state
  function showStatusOverlay() {
    ...
    lines.forEach(([k, v]) => {
      ...
      key.style.color = "#7dd8ff";
      key.textContent = k;
      const val = document.createElement("span");
      val.style.color = v.includes("NOT") ? "#ff6b6b" : "#a8ff78";
      val.textContent = v;
      row.appendChild(key);
      row.appendChild(val);
      ov.appendChild(row);
  ...
  function setup() {
    ...
    // ─── Hide CyTube default chat column ─────────────────────────────────
    $("#chatwrap").hide();
    ...
    $("head").append(`<style id="kl-styles">
      #kl-nav {
        ...
        font-family:sans-serif; z-index:9998; gap:8px;
        user-select:none;
      }
      #kl-nav-count {
    ...
    function processNode(node) {
      ...
      // Extract message — last span with no class (not timestamp, not name wrapper)
