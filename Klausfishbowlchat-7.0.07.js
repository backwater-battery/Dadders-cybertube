// Klaus Fish Bowl Chat — v7.0.07
// GitHub : https://github.com/backwater-battery/Dadders-cybertube
// jsDelivr: https://cdn.jsdelivr.net/gh/backwater-battery/Dadders-cybertube@main/Klausfishbowlchat-7.0.07.js

(function () {
  "use strict";

  // ── Config ─────────────────────────────────────────────────────────────────
  var KLAUS_DEBUG = true;   // set false to hide Klaus's Logs button

  // ── Deterministic colour system ────────────────────────────────────────────
  // colour = PALETTE[ djb2(username) % PALETTE.length ]
  // Pure hash — same result on every client for the same username.
  // CyTube's own userlist colour (server-assigned) is used when available
  // as it is also identical across all clients.
  var PALETTE = [
  ...
  function getColor(name) {
    ...
    var found = null;
    document.querySelectorAll("#userlist .userlist_item").forEach(function(div) {
      var nameSpan = div.children[1];
      if (nameSpan && nameSpan.textContent.trim() === name) {
        var c = $(nameSpan).css("color");
  ...
  function buildLogUI() {
    ...
    titleEl.textContent = "🪵 Klaus's Logs";
    titleEl.style.cssText = "font-weight:700;font-size:11px;color:#00ff88;flex:1;";
    ...
    var copyBtn = makeBtn("Copy", "#a8ff78", function() {
    ...
    var clearBtn = makeBtn("Clear", "#ffcc44", function() { logLines = []; logBody.innerHTML = ""; dbg("log cleared"); });
    var closeBtn = makeBtn("✕",     "#ff6b6b", function() { logPanel.style.display = "none"; });
    var statBtn  = makeBtn("Status","#7dd8ff", showStatusOverlay);
  ...
  function showStatusOverlay() {
    var existing = document.getElementById("kl-status-overlay");
    ...
    var scoreEntries = Object.keys(boatScores)
      .sort(function(a,b){ return boatScores[b]-boatScores[a]; })
      .map(function(n){ var s=boatScores[n]; return n+": "+s+" ("+["🚣","⚡","🤿"][getPower(s)]+")"; })
      .join(" | ") || "none yet";
    ...
    rows.forEach(function(pair) {
      ...
      var kEl = document.createElement("span"); kEl.style.cssText = "color:#7dd8ff;flex-shrink:0"; kEl.textContent = pair[0];
      var vEl = document.createElement("span"); vEl.style.cssText = "color:"+(pair[1].indexOf("NOT")!==-1?"#ff6b6b":"#a8ff78")+";text-align:right;word-break:break-all"; vEl.textContent = pair[1];
    ...
    dismissBtn.addEventListener("click", function() { ov.remove(); });
  ...
  function init() {
    ...
    statusMsg = "running";
    dbg("init OK — socket ready");
    setup();
  }

  // Small delay so CyTube's own init finishes first
  setTimeout(init, 500);
  ...
  function setup() {
    ...
    // Background
    document.body.style.backgroundImage      = "url('https://i.ibb.co/TDKkr0rx/Klaus-Fish-Bowl-Chats.webp')";
    ...
    // Hide CyTube's default chat column — keep #chatline in DOM
    $("#chatwrap").hide();
    ...
    styleEl.textContent = [
      "#kl-nav{position:fixed;top:0;left:0;right:0;height:42px;background:rgba(0,0,0,0.75);border-bottom:1px solid rgba(0,180,255,0.2);display:flex;align-items:center;padding:0 14px;color:#7dd8ff;font-weight:700;font-size:14px;font-family:sans-serif;z-index:9998;gap:8px;user-select:none;}",
      "#kl-nav-count{margin-left:auto;font-size:12px;font-weight:400;color:rgba(130,200,255,0.55);}",
      "#kl-water{position:fixed;top:42px;left:0;right:0;height:52px;background:rgba(0,30,80,0.55);border-bottom:2px solid rgba(80,180,255,0.35);z-index:810;overflow:hidden;pointer-events:none;}",
      "#kl-water::after{content:'';position:absolute;bottom:0;left:0;right:0;height:4px;background:rgba(100,200,255,0.25);}",
      ".kl-boat{position:absolute;bottom:6px;display:flex;flex-direction:column;align-items:center;gap:1px;pointer-events:none;animation:kl-bob 2.4s ease-in-out infinite;}",
      ...
      "#kl-bubbles{position:fixed;top:94px;left:0;right:0;bottom:110px;pointer-events:none;overflow:hidden;z-index:800;}",
      ".kl-bubble{position:absolute;padding:7px 14px;border-radius:22px;font-size:13px;font-weight:600;color:#fff;white-space:nowrap;max-width:300px;overflow:hidden;text-overflow:ellipsis;font-family:sans-serif;pointer-events:none;line-height:1.4;}",
      ".kl-pop{position:fixed;font-size:20px;z-index:820;pointer-events:none;animation:kl-pop-anim 0.55s ease-out forwards;}",
      "@keyframes kl-pop-anim{0%{opacity:1;transform:scale(1) translateY(0)}100%{opacity:0;transform:scale(2.2) translateY(-18px)}}",
      ".kl-powerup{position:fixed;z-index:830;pointer-events:none;font:700 12px sans-serif;color:#ffcc44;text-shadow:0 0 6px rgba(0,0,0,0.8);animation:kl-pu-anim 1.4s ease-out forwards;}",
      "@keyframes kl-pu-anim{0%{opacity:1;transform:translateY(0)}100%{opacity:0;transform:translateY(-30px)}}",
      "#kl-chatbar{position:fixed;bottom:0;left:0;right:0;padding:6px 10px 8px;background:rgba(0,0,0,0.82);border-top:1px solid rgba(0,150,255,0.15);z-index:9998;display:flex;flex-direction:column;gap:5px;font-family:sans-serif;}",
    ...
    // Remove old style block if re-running
    var oldStyle = document.getElementById("kl-styles");
    if (oldStyle) oldStyle.remove();
    document.head.appendChild(styleEl);

    // Remove any old UI elements if re-running
    ["kl-nav","kl-water","kl-bubbles","kl-chatbar"].forEach(function(id) {
    ...
    nav.id = "kl-nav";
    nav.innerHTML = '<span style="font-size:20px;line-height:1">🐟</span> Klaus Fish Bowl Chats — Dadders <span id="kl-nav-count"></span>';
    document.body.appendChild(nav);

    function syncCount() {
      var nc = document.getElementById("kl-nav-count");
      var uc = document.getElementById("usercount");
      if (nc && uc) nc.textContent = uc.textContent;
    }
    syncCount();
    ...
    bubbleLayer.id = "kl-bubbles";
    document.body.appendChild(bubbleLayer);

    // Chat bar
    var chatbar = document.createElement("div");
    chatbar.id = "kl-chatbar";
    chatbar.innerHTML = [
      '<div id="kl-userlist-row"><span class="kl-ul-label">online:</span></div>',
      '<div id="kl-inputrow">',
      '<input id="kl-input" type="text" placeholder="Message the fish bowl…" autocomplete="off" autocorrect="off" spellcheck="false" maxlength="240" />',
    ...
    function renderBoat(name) {
      ...
      var score = boatScores[name] || 0;
      var power = getPower(score);
      var bx    = getBoatX(name);
      ...
      if (power === 2) {
        var subHull = document.createElement("div");
        subHull.className = "kl-boat-sub-hull";
        subHull.style.background = hexToRgba(color, 0.82);
        subHull.style.border = "1.5px solid " + color;
        subHull.textContent = "🤿 " + name;
        boat.appendChild(subHull);
      } else {
    ...
    function showPowerUp(x, y, text) {
      ...
      setTimeout(function() { pu.remove(); }, 1500);
    }
