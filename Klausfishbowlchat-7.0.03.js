...
// ═══════════════════════════════════════════════════════════════════════════════
//  Klaus Fish Bowl Chat — v7.0.03
//  GitHub : https://github.com/backwater-battery/Dadders-cybertube
//  jsDelivr: https://cdn.jsdelivr.net/gh/backwater-battery/Dadders-cybertube@main/Klausfishbowlchat-7.0.03.js
//
//  CHANGES v7.0.03:
//    • FIXED userlist — CyTube uses div.userlist_item (not li.userlist_item)
//      confirmed from util.js addUserToList():
//        $("<div/>").addClass("userlist_item")
//        children[0] = rank icon span
//        children[1] = name span (plain span, no class)
//    • FIXED $server$ filter — was failing due to $ in regex, now uses
//      plain string includes() check instead
// ═══════════════════════════════════════════════════════════════════════════════
...
(function () {
  ...
  function buildLogUI() {
    ...
    function makeBtn(label, color, onClick) {
      ...
      b.style.cssText = [
        ...
        "font:700 9px monospace","cursor:pointer","flex-shrink:0",
      ].join(";");
  ...
  function showStatusOverlay() {
    ...
    const rows = [
      ...
      ["userlist divs", String(document.querySelectorAll("#userlist .userlist_item").length)],
    ];
    rows.forEach(([k, v]) => {
  ...
  function init() {
    ...
    statusMsg = "running";
    dbg("v7.0.03 init OK — socket ready");
    setup();
  }
  ...
  function setup() {
    ...
    function getColor(name) {
      ...
      // Read from userlist — confirmed structure:
      //   #userlist > div.userlist_item > span[0]=icon, span[1]=name
      let found = null;
    ...
    // ── Userlist sync ─────────────────────────────────────────────────────
    // CONFIRMED from util.js addUserToList():
    //   $("<div/>").addClass("userlist_item")   ← div, NOT li
    //   children[0] = rank icon span
    //   children[1] = name span (plain span, no class, text = data.name)
    // Color is applied later by formatUserlistItem() via jQuery .css("color")
    function refreshUserList() {
      ...
      dbg("userlist: " + items.length + " items");
      ...
      if (n > 0) dbg("userlist: rendered " + n + " pills");
    }
    ...
    if (ulEl) {
      new MutationObserver(refreshUserList)
        .observe(ulEl, { childList: true, subtree: true });
    }
    ...
    //
    //  Server/system messages to SKIP — identified by class or username:
    //
    //  Class-based skip (string includes, no regex — avoids $ issues):
    //    "nick-highlight"     mention highlights
    //    "server-msg"         server status divs (connect/disconnect/etc)
    //
    //  Name-based skip (after username extraction):
    //    "$server$"           CyTube system announcements (/clear, voteskip, etc)
    //    "$voteskip$"         voteskip messages
    //    "$poll$"             poll results
    //
    const SKIP_CLASS_FRAGMENTS = ["nick-highlight", "server-msg"];
    ...
    function processNode(node) {
      ...
      // Skip system messages — use plain string includes, not regex
      // (regex would need escaping for $ in class names like chat-msg-$server$)
      if (SKIP_CLASS_FRAGMENTS.some(f => cls.includes(f))) {
        dbg("skip system cls: " + cls.substring(0,40));
        return;
      }

      // Also skip if class contains "$server$" directly
      if (cls.includes("$server$")) {
        dbg("skip $server$ cls");
        return;
      }
      ...
      // Extract username — priority order:
      //  1. strong.username text (present when user changes or first message)
      //  2. "chat-msg-{name}" class fragment
      //  3. lastSeenName (same user back-to-back, CyTube omits the name span)
      let name = "";
      ...
      if (!name) {
        // Use indexOf instead of regex to avoid issues with special chars in names
        const prefix = "chat-msg-";
        const idx = cls.indexOf(prefix);
        if (idx !== -1) {
          // Extract up to next space or end of string
          const rest = cls.slice(idx + prefix.length);
          name = rest.split(" ")[0];
          if (name) lastSeenName = name;
        }
      }
      if (!name && lastSeenName) name = lastSeenName;
      if (!name) { dbg("  skip: no name"); return; }

      // Skip server pseudo-usernames
      if (SKIP_NAMES.includes(name)) {
        dbg("  skip name: " + name);
        return;
      }

      // Extract message text — last span that isn't timestamp or name wrapper
      ...
      // Fallback: strip "Name: " prefix from full node text
      if (!msgText) {
